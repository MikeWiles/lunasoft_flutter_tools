...
class MWImage extends StatelessWidget {

  MWImage({this.imageUrl, this.size, this.padding = 0, this.hasBorder = false, this.borderColour, this.icon, this.localFile, this.isOval = false, this.height, this.width});
        ...
        return CachedNetworkImage(
          ...
          placeholder: (context, url) => Container(
            ...
            child:SizedBox(
                height: height ?? size,
                width: height ?? size,
                child: CircularProgressIndicator(valueColor: AlwaysStoppedAnimation<Color?>(borderColour ?? Theme.of(context).primaryColor))
            )
          ),
          errorWidget: (context,url,error) => _getErrorWidget()!,
        );
      } if(imageUrl!.toLowerCase().contains(".png") || imageUrl!.toLowerCase().contains(".jpg") || imageUrl!.toLowerCase().contains(".jpeg")) {
